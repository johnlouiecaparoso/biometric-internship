  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      {section('Notifications', <Bell className="w-4 h-4" />, <>
        {row('Absence Alerts', 'Send alert when intern has not timed in by 10:00 AM', notifs.absenceAlert, () => setNotifs(n => ({ ...n, absenceAlert: !n.absenceAlert })))}
        {row('Late Arrival Alerts', 'Notify admin when intern arrives past grace period', notifs.lateAlert, () => setNotifs(n => ({ ...n, lateAlert: !n.lateAlert })))}
        {row('Completion Alerts', 'Alert when intern is near or reaches required hours', notifs.completionAlert, () => setNotifs(n => ({ ...n, completionAlert: !n.completionAlert })))}
        {row('Weekly Summary Report', 'Receive automated weekly attendance report', notifs.weeklyReport, () => setNotifs(n => ({ ...n, weeklyReport: !n.weeklyReport })))}
        {row('Pending Requests Alert', 'Notify when correction requests need review', notifs.pendingRequest, () => {
          ...
          setNotifs(n => ({ ...n, pendingRequest: next }));
          if (next && browserPermission !== 'granted') requestBrowserPermission();
        })}
      </>)}

      {section('Intern Policies', <Users className="w-4 h-4" />, <>
        {row('Allow Correction Requests', 'Enable interns to submit attendance correction requests', system.allowCorrections, () => setSystem(s => ({ ...s, allowCorrections: !s.allowCorrections })))}
